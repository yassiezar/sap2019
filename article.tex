...
To this end, we are developing a mobile navigation system using a Google Project Tango tablet device\footnote{https://developers.google.com/tango}, pictured in \cref{fig:tango-headphone}, that caters to the perception limitations of people with vision impairments by using a non-visual interface.
...
For this work, we implement and evaluate an audio guidance interface that will be implemented in the guidance system to provide navigation instructions to the user. 
...
With these limits, $f$ can be determined with \cref{eq:frequency}, where $m$ is the given setting's gradient while $c$ can be found using the current setting's pitch and angle limits.
